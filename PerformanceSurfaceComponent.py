...
from .XYPadSectionComponent import XYPadSectionComponent
...
class PerformanceSurfaceComponent(object):
    ...
    def __init__(self, matrix, side_buttons, top_buttons, control_surface,
        ...
        self._xy_section = XYPadSectionComponent(
            matrix=self._top_matrix,
            navigation_provider=self._blue_hand_navigation,
            track_color_provider=self._xy_track_color_value)
        self._overlay = PerformanceConfigOverlayComponent(
            matrix=self._matrix,
            control_surface=self._control_surface,
            on_layout_selected=self._on_layout_selected)

        if hasattr(self._instrument_controller, 'set_scales_overlay_callback'):
            self._instrument_controller.set_scales_overlay_callback(self._on_scales_overlay_changed)
    ...
    def disconnect(self):
        ...
        self._xy_section.disconnect()
        self._pads_section.disconnect()
        if hasattr(self._instrument_controller, 'set_scales_overlay_callback'):
            self._instrument_controller.set_scales_overlay_callback(None)
        self._instrument_controller = None
        self._pads_section = None
        self._xy_section = None
    ...
    def _disable_surface(self):
        ...
        self._xy_section.set_enabled(False)
    ...
    def _update_active_layout(self):
        ...
        if self._layout_id == STACKED_XY_PADS_LAYOUT:
            self._update_stacked_xy_pads_layout()
        else:
            self._update_full_pads_layout()

        self._update_layout_osd()

    def _update_full_pads_layout(self):
        self._xy_section.set_enabled(False)
        ...
        self._update_pads_section()

    def _update_stacked_xy_pads_layout(self):
        self._pads_section.set_matrix(self._bottom_matrix, full_matrix=self._matrix)
        self._pads_section.set_physical_note_layout(4, self._matrix.height())
        self._apply_navigation_layout()
        self._update_pads_section()

        if self._pads_section.is_scale_overlay_active:
            self._xy_section.set_enabled(False)
        else:
            self._xy_section.set_matrix(self._top_matrix)
            if not self._xy_section.is_enabled():
                self._xy_section.set_enabled(True)
            else:
                self._xy_section.update()

        self._blue_hand_navigation.update_device_buttons()

    def _update_pads_section(self):
    ...
    def _update_layout_osd(self):
        if self._osd is None or self._pads_section.is_scale_overlay_active:
            return
    ...
    def _on_scales_overlay_changed(self, is_enabled):
        if self._enabled and not self._config_mode:
            self.update()

    def _xy_track_color_value(self):
        if self._pads_section is None:
            return None
        return self._pads_section.track_pad_color_value